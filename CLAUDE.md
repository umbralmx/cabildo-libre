...
>
> **The panel is an Observable Framework app in modo instrumento (2026-09-05).** The style
> guide's surface table assigns the dashboard surface to Framework (ADR-0004, v1.4.0) and its
> mode to **instrumento**; the buscador and the methodology page are *web* surfaces and stay in
> **laboratorio**. Source in `panel/`, built to `site/panel/` (gitignored — CI builds it), served
> at `…/cabildo-libre/panel/`. `site/panel.html` is now a redirect. See `docs/diseno.md`.
>
...
- **The mode is set by the medium, not by the reader's OS (UMB-COL-011).** The surface table decides: `site/` (buscador, methodology) is a *web* surface → **laboratorio**; `panel/` is a *dashboard* → **Observable Framework, instrumento**. Do not add a `prefers-color-scheme` pair, and do not put a chart-heavy dashboard on the web surface.
