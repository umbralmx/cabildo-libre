...
>
> **The whole instrument is modo instrumento (2026-09-05).** Every page — buscador, methodology
> and panel — carries `data-mode="instrumento"` on its `<html>`. This is a **deliberate deviation**
> from the guide's surface table, which puts `web` in laboratorio; the reasoning is in
> `docs/diseno.md` and it is the same one desaparecidosmx used to reverse this call: cabildo-libre
> is a monitor over a public register, not a page about a project. What UMB-COL-011 does require —
> one mode per artifact, set by the medium and never by `prefers-color-scheme` — is satisfied, and
> `verificar_marca.py` checks it page by page. The switch cost no colour rule: every colour in
> `styles.css` is a token.
>
> **The panel is an Observable Framework app.** The guide assigns the dashboard surface to
> Framework (ADR-0004, v1.4.0). Source in `panel/`, built to `site/panel/` (gitignored — CI builds
> it), served at `…/cabildo-libre/panel/`. `site/panel.html` is now a redirect.
>
...
| `site/metodologia.html` + `metodologia.js` | **The public-facing methodology page** — the method for a non-technical audience. Every figure reads from `analytics-<termino>.json`; never hardcode a number in it |
...
| `panel/` + `observablehq.config.js` | **The dashboard**, an Observable Framework app in modo instrumento. `npm run build` → `site/panel/`. Charts are Observable Plot through `@umbralmx/umbral-plot` (vendored in `vendor/`, not on npm) |
...
- **Everything is modo instrumento, and that is one mode for the whole artifact (UMB-COL-011).** Every page carries `data-mode="instrumento"` in its HTML — never set by JavaScript, or the page flashes light first. This deviates from the surface table (`web` → laboratorio) on purpose; see `docs/diseno.md`. Never add a `prefers-color-scheme` pair, and never leave one page behind in laboratorio: half-light/half-dark is the exact defect the rule exists to prevent.
- **The panel's mode lives in three places and they must agree:** `MODE` in `panel/components/format.js`, `data-mode` written into the built HTML by `scripts/copy-static.mjs` (never by JS, or the page flashes light first), and the dark isotype. `verificar_marca.py` checks all three.
- **`site/panel/` is build output, not source.** It is gitignored and built in CI. Never commit it, and never hand-edit `panel/observable-framework-instrumento.css` — it is generated by the style guide; replace it, don't edit it.
